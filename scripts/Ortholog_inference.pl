...
my %opts;
GetOptions(\%opts,"g=s","r=s","o=s","psonic=s","h|help");
if (!( defined $opts{g} and defined $opts{r} and defined $opts{o} and defined $opts{psonic})) {
		die "************************************************\n
	...
	-psonic Full path the [pSONIC.py] script provided by pSONIC software
	Optional:
...
chdir "$opts{o}";
system "python $opts{psonic}/pSONIC.py data translate_gff -gff all.gff";
system "MCScanX data -b 2";
system "python $opts{psonic}/pSONIC.py data";
