...
use Getopt::Long;
my %opts;
GetOptions(\%opts,"i=s","m=s","n=s","gff=s","o2=s","pairs2=s","h|help");
if (!(defined $opts{i} and defined $opts{m} and defined $opts{n} and defined $opts{gff} and defined $opts{pairs2} and defined $opts{o2})) {
		die "************************************************\n
...
sub get_matched_scaff{
    ...
    close IN;
    open O,"> $opts{o2}/$nv_cg/$name.scaff.match";
...
sub sort_get_table{
    ...
    my %hash=();
    my %store2=();
    open IN,"<$opts{o2}/$nv_cg/$name.scaff.match" or die;
    while(my $a=<IN>){
	    chomp $a;
	    my @a=split/\t/,$a;
    ...
    open O,"> $opts{o2}/$nv_cg/$name.scaff.match.table";
    my %final=();
