...
if (!( defined $opts{i1} and defined $opts{o1})) {
		die "************************************************\n
	-i1	The parameter refers to the same parameter [-i] used in the previously executed script [Macrosyn1.pl]
	-o1	Full path to the [outputDir1] directory containing output files (same as [-o1] in the previous script [Macrosyn1.pl])
	Optional:
...
system("Rscript Gene_families.R $opts{o1}/ancient_gene_families.analysis/family_situation.table  $opts{o1}/inputR_barchart_data $opts{i1}/tree.nwk $opts{o1} $opts{i1}/species_classification.table $opts{width} $opts{height}");
