...
if (!( defined $opts{node} and defined $opts{n1} and defined $opts{n2} and defined $opts{orth} and defined $opts{o})) {
		die "************************************************\n
	-node	Full path to the [node.result] file generated by [Count] software
	-n1	The name of the interested ancestor node represented by a number (example: 1)
	-n2	The name of the ancestor node (that is one level above the ancestor node of interest) represented by a number (example: 2)
	-orth	Full path to the [Orthogroup_Sequences] directory generated by [OrthoFinder]
...
open O4,">$opts{o}/node$opts{n1}-Loss.OG"; 
...
open O2,">$opts{o}/node$opts{n1}-All.OG.genes";
...
open O2,">$opts{o}/node$opts{n2}-All.OG.genes";
while (my $a=<I>) {
	chomp $a;
	open I1,"<$opts{orth}/$a.fa"; 
	while (my $a2=<I1>) {
...
open O1,">$opts{o}/node$opts{n1}-Gain.OG.fa";
open I,"<$opts{o}/node$opts{n1}-Gain.OG"; 
open O2,">$opts{o}/node$opts{n1}-Gain.OG.genes";
while (my $a=<I>) {
	chomp $a;
	open I1,"<$opts{orth}/$a.fa"; 
	while (my $a2=<I1>) {
...
open O1,">$opts{o}/node$opts{n1}-Loss.OG.fa";
open I,"<$opts{o}/node$opts{n1}-Loss.OG"; 
open O2,">$opts{o}/node$opts{n1}-Loss.OG.genes";
while (my $a=<I>) {
	chomp $a;
	open I1,"<$opts{orth}/$a.fa"; 
	while (my $a2=<I1>) {
...
close O1;
close O2;
system "cat $opts{o}/node$opts{n1}-Loss.OG.fa $opts{o}/node$opts{n1}-Gain.OG.fa >$opts{o}/for_emapper.pep";
