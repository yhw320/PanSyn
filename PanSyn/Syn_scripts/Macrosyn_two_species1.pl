...
use Getopt::Long;
my %opts;
GetOptions(\%opts,"pairs=s","m=s","n=s","gffm=s","gffn=s","o=s","len=s","h|help");
if (!(defined $opts{pairs} and defined $opts{m} and defined $opts{n} and defined $opts{gffm} and defined $opts{gffn} and defined $opts{len} and defined $opts{o})) {
		die "************************************************\n
	...
	-len	Interested species I' length file for each chromosomes (example: HSap.len)
...
if (defined $opts{h} or defined $opts{help}) {
	...
	-gffm	Interested species I' GFF file (example: HSap.gff)
	-gffn	Interested species II' GFF file (example: MMus.gff)
	-len	Interested species II' length file for each chromosomes (example: MMus.len)
	-o	Full path to the new folder storing the output results
	-h|-help Print this help page
		*************************************************\n";
}
...
system"mkdir $opts{o}/$nv_cg";
	
open IN,"<$opts{len}" or die("Could not open $opts{len}.\n"); 
open O,">$opts{o}/$nv_cg/$opts{m}.block" or die("$opts{o}/$nv_cg/$opts{m}.block.\n"); 
$n=0;
while(<IN>){
	chomp;
	my @a=split /\t/;
...
close IN;
close O;
$pathway_b="$opts{o}/$nv_cg/$opts{m}.block";
...
sub get_matched_scaff{
    ...
    close IN;
    open O,"> $opts{o}/$nv_cg/$name.scaff.match";
...
sub sort_get_table{
    ...
    my %hash=();
    my %store2=();
    open IN,"<$opts{o}/$nv_cg/$name.scaff.match" or die;
    while(my $a=<IN>){
	    chomp $a;
	    my @a=split/\t/,$a;
    ...
    open O,"> $opts{o}/$nv_cg/$name.scaff.match.table";
    my %final=();
