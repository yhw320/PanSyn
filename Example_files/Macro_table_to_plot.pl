...
use lib;
use SVG;
GetOptions(\%opts,"i1=s","m=s","n=s","e=s","c=s","color=s","gff=s","g=s","p=s","name1=s","name2=s","o4=s","h|help");
if (defined $opts{h} or defined $opts{help}) {
	...
	-o4	The parameter refers to the same parameter [-o4] in the previously executed script [Pairs_to_table.pl])
	...
	-color	Full path to the ancestral chromosome color file
	Optional:
...
if (!(defined $opts{i1} and defined $opts{m} and defined $opts{color} and defined $opts{n} and defined $opts{gff} and defined $opts{g}  and defined $opts{o4})) {
		die "************************************************\n
	-i1	The parameter refers to the same parameter [-i1] used in the previously executed script [Macrosyn1.pl]
	-m	Enter the abbreviation for the name of the species representing the ancestral genome (example: NVec)
	-n	Enter the abbreviation for the name of the interested species (example: HSap)
...
my $pathway_pepm="$opts{i1}/peps/$spename1.pep";
my $pathway_pepn="$opts{i1}/peps/$spename2.pep";
...
sub getseqpair{
	...
	if (-d "$opts{o4}/$nv_cg/Macrosyn_genes") {
		print "The $opts{o4}/$nv_cg/Macrosyn_genes directory already exists!\nThe old directory has been deleted.\n";
		system "rm -r $opts{o4}/$nv_cg/Macrosyn_genes";
	}

	system "mkdir $opts{o4}/$nv_cg/Macrosyn_genes";
	print "The [$opts{o4}/$nv_cg/Macrosyn_genes] directory has been created!\n";
	...
	if ($c_chmun==1) {
		...
		open G,"<$gff1";
		open O,">$opts{o4}/$nv_cg/Macrosyn_genes/$opts{m}.pair";
		while ($a=<G>){
			chomp $a;
			my @items=split/\t/,$a;
			open B,"<$bl";
		...
		sub c_scaffold2{
		open I1,"<$opts{o4}/$nv_cg/$spename2\_chr2scaf.info"; 
		$lv=1;
		while(<I1>){
			chomp;
			my @ar=split/\t/,$_;
		...
		close I1;
		open I2,"<$opts{o4}/$nv_cg/$spename2\_chr2scaf.info"; 
		open O,">$opts{o4}/$nv_cg/Macrosyn_genes/scaf_out"; 
		while(<I2>){
			chomp;
			my @ar2=split/\t/,$_;
		...
		open M,"<$opts{o4}/$nv_cg/Macrosyn_genes/$opts{m}.pair";
		while ($a=<M>){
			chomp $a;
			my @items=split/\t/,$a;
			$h1{$items[1]}=$items[0];
		}
		open I,"<$opts{o4}/$spename1\_$spename2.pairs2.score";
		open O,">$opts{o4}/$nv_cg/Macrosyn_genes/pair3";
		while ($b=<I>){
			chomp $b;
			my @items2=split/\t/,$b;
			$b2=$items2[0];
		...
		sub id_chr_match2{
		open G,"<$pathway_gffn";
		open O,">$opts{o4}/$nv_cg/Macrosyn_genes/spe2.pair";
		while ($a=<G>){
			chomp $a;
			my @items=split/\t/,$a;
		...
		open I,"<$opts{o4}/$nv_cg/Macrosyn_genes/spe2.pair";
		open O,">$opts{o4}/$nv_cg/Macrosyn_genes/$opts{m}_$opts{n}_all_gene_pairs.dot";
		...
		open M,"<$opts{o4}/$nv_cg/Macrosyn_genes/scaf_out";
		while ($c=<M>){
			chomp $c;
			my @items2=split/\t/,$c;
		...
		open N,"<$opts{o4}/$nv_cg/Macrosyn_genes/pair3";
		while ($n=<N>){
			chomp $n;
			my @items4=split/\t/,$n;
			$i2=$items4[1];
		...
		system "rm $opts{o4}/$nv_cg/Macrosyn_genes/$opts{m}.pair";
		system "rm $opts{o4}/$nv_cg/Macrosyn_genes/scaf_out";
		system "rm $opts{o4}/$nv_cg/Macrosyn_genes/pair3";
		system "rm $opts{o4}/$nv_cg/Macrosyn_genes/spe2.pair";
		...
		open I,"<$opts{o4}/$nv_cg/Macrosyn_genes/$opts{m}_$opts{n}_all_gene_pairs.dot";
		open O1,">$opts{o4}/$nv_cg/Macrosyn_genes/$opts{m}_all_gene_pairs.pep";
		open o4,">$opts{o4}/$nv_cg/Macrosyn_genes/$opts{n}_all_gene_pairs.pep";
		while ($b=<I>){
			chomp $b;
			@items=split/\t/,$b;
		...
		close I;close P1;close P2;close O1;close o4;
		}
		get_dia_gene();
		...
		sub get_dia_gene{
			...
			close Y;
			system "awk -F '\t'  '{for(i=0;++i<=NF;)a[i]=a[i]?a[i] FS \$i:\$i}END{for(i=0;i++<NF;)print a[i]}' $opts{o4}/$nv_cg/$final > $opts{o4}/$nv_cg/Macrosyn_genes/try";
			my $max=0;my $num_max=0;my @items;my $num;my $n;my %h1;my %h2;my $fir;my @items2;my %h3;
			open IN,"<$opts{o4}/$nv_cg/Macrosyn_genes/try";
			open O,">$opts{o4}/$nv_cg/Macrosyn_genes/$opts{m}_$opts{n}_diagonal_gene_pairs.dot";
			open O1,">$opts{o4}/$nv_cg/Macrosyn_genes/$opts{m}_$opts{n}_significant_gene_pairs.dot";
			print O "Ancestor-geneID\tspecies-geneID\tAncestor-chr\tspecies-chr\n";
			...
			close IN;
			system "rm $opts{o4}/$nv_cg/Macrosyn_genes/try";
			open IN2,"<$opts{o4}/$nv_cg/Macrosyn_genes/$opts{m}_$opts{n}_all_gene_pairs.dot";
			while (my $a=<IN2>){
				chomp $a;
				@items2=split/\t/,$a;
		...
		open I,"<$opts{o4}/$nv_cg/Macrosyn_genes/$opts{m}_$opts{n}_diagonal_gene_pairs.dot";
		open O1,">$opts{o4}/$nv_cg/Macrosyn_genes/$opts{m}_diagonal_gene_pairs.pep";
		open o4,">$opts{o4}/$nv_cg/Macrosyn_genes/$opts{n}_diagonal_gene_pairs.pep";
		while ($b=<I>){
			chomp $b;
			@items=split/\t/,$b;
			if (exists $h1{$items[0]}) {
	...
	if ($c_chmun==2) {
		...
		open I,"<$opts{o4}/$nv_cg.pairs2.score";
		open O,">$opts{o4}/$nv_cg/Macrosyn_genes/pair3";
		while ($b=<I>){
			chomp $b;
			@items2=split/\t/,$b;
		...
		open M,"<$opts{o4}/$nv_cg/$spename2\_chr2scaf.info";
		while ($c=<M>){
			chomp $c;
			@items2=split/\t/,$c;
			foreach my $ii(@items2) {
		...
		#system "perl get_dia_gene.pl -matrix /public/home/ws/ws_data/yuhw/synteny/connect86/result2/PY_CG/PY_CG_scaff_c3 -o /public/home/ws/ws_data/yuhw/synteny/connect86/result/tiqv";
		sub get_dia_gene1{
		system "awk -F '\t'  '{for(i=0;++i<=NF;)a[i]=a[i]?a[i] FS \$i:\$i}END{for(i=0;i++<NF;)print a[i]}' $opts{o4}/$nv_cg/$nv_cg.scaff.match.table > $opts{o4}/$nv_cg/Macrosyn_genes/try";
		my $max=0;my $num_max=0;my @items;my $num;my $n;my %h1;my %h2;my $fir;my @items2;my %h3;
		open IN,"<$opts{o4}/$nv_cg/Macrosyn_genes/try";
		open O,">$opts{o4}/$nv_cg/Macrosyn_genes/$opts{m}_$opts{n}_diagonal_gene_pairs.dot";
		print O "Ancestor-geneID\tspecies-geneID\tAncestor-chr\tspecies-chr\n";
...
open O,">$opts{o4}/formatted_ancestor_chr_color.txt";
my %h_un=();my %h_numpai=();
