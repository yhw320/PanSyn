...
while (my $c=readdir P) {
	if ( $c=~/^(\S+).pep$/ ) {
		...
		if ($cf_name ne $opts{a}) {
			my $pep_file=$dir1."/$c";
			system "blastp -query $py_pep -db $pep_file -outfmt 6 -out $opts{o}/$opts{a}_$cf_name.blastp -evalue $opts{e} -num_threads $opts{t}";
			system "blastp -query $pep_file -db $py_pep -outfmt 6 -out $opts{o}/$cf_name\_$opts{a}.blastp -evalue $opts{e} -num_threads $opts{t}";
