...
if(!(defined $opts{i} and defined $opts{o2} and defined $opts{b} and defined $opts{c} and defined $opts{color})){
	die"**********************************************\n
	-i	The parameter refers to the same parameter [-i] used in the previously executed command [Microsyn_cluster]
	-o2	Full path to the new [outputDir] directory containing output files	
	-b	Full path to the interested [*_microsyn_genes.blocks] directory (generated by the previous command [Microsyn_cluster])
