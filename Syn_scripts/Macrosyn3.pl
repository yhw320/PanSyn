...
use SVG;
GetOptions(\%opts,"i=s","m=s","n=s","e=s","c=s","gff=s","g=s","l=s","s=s","p=s","color=s","name1=s","name2=s","o=s","h|help");
...
if (!(defined $opts{i} and defined $opts{m} and defined $opts{n} and defined $opts{color} and defined $opts{gff} and defined $opts{g} and defined $opts{s} and defined $opts{o})) {
		die "************************************************\n
	-i	Full path to the folder containing required data files (same as [-i] in the previous script [Macrosyn1_1_blast.pl])
	-m	Selected Ancestor'name by abbreviations (example: NVec)
	-n	Interested species'name byabbreviations (example: HSap)
