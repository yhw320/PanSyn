...
if (!(defined $opts{m} and defined $opts{n}  and defined $opts{color} and defined $opts{i} and defined $opts{g} and defined $opts{gff} and defined $opts{pep} and defined $opts{s} and defined $opts{f} and defined $opts{o})) {
	...
	-evalue	diamond alignment evalue (default:1e-3)
...
$fileExist = -e "$opts{f}/add_species/$opts{m}/all.peps.database.dmnd";
if ( $fileExist ) {
}
else {
system "diamond makedb --in $opts{f}/add_species/$opts{m}/all.peps -d $opts{f}/add_species/$opts{m}/all.peps.database";
}



system "diamond blastp --query $opts{pep} --db $opts{f}/add_species/$opts{m}/all.peps.database --outfmt 6 --out $opts{o}/$opts{m}\_$opts{n}/$opts{n}_all.blast --evalue $opts{evalue} --threads $opts{t}";
system "diamond blastp --query $opts{f}/add_species/$opts{m}/all.peps --db $opts{pep}.database --outfmt 6 --out $opts{o}/$opts{m}\_$opts{n}/all_$opts{n}.blast  --evalue $opts{evalue} --threads $opts{t}";
