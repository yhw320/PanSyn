...
if (!( defined $opts{o2} and defined $opts{node} and defined $opts{tre} and defined $opts{n})) {
		die "************************************************\n
	-o2	The parameter refers to the same parameter [-o2] used in the previously executed script [Microsyn_relaxed_B1.pl]
...
if (defined $opts{h} or defined $opts{help}) {
		die "************************************************\n
	-o2	Full path to the new folder containing results (same as [-o2] in the previous script [Microsyn_relaxed_B1.pl])
	-node	The name of the internal node at which to identify con-served microsynteny gene clusters (example: Ancestor1)
	-tre	Full path to the [*.tre] file
	-n	The distance threshold above which syntenic blocks will be split (an appropriate threshold in the xleft column of the [nmax_test.inflection.csv] file)
	optional:
...
#chdir "$opts{o2}";
system "python step3_find_og_commus.py $opts{o2}/$opts{node}-myresults/bynode/$opts{node}/m_$opts{spenum2}/$opts{node}-myresults.m_$opts{spenum2}.$opts{node}.dist  -n $opts{n} -o $opts{o2}/$opts{node}-myresults/bynode/$opts{node}.og_commus";

system "python step4_OG_communities_to_blocks_graph_check.py $opts{o2}/$opts{node}-myresults/bynode/$opts{node}.og_commus.csv -g $opts{o2}/$opts{node}-myresults/bynode/$opts{node}.og_commus.gpickle -c $opts{o2}/$opts{node}-myresults/chromdata.pickle -o $opts{o2}/$opts{node}-myresults/$opts{node}_blocks -l $opts{pl} -s $opts{ps} -k $opts{pk} -r $opts{pr}";

system "python BlocksByNode.py -c $opts{o2}/$opts{node}-myresults/$opts{node}_blocks.len$opts{pl}.ol$opts{ps}.clusters -b $opts{o2}/$opts{node}-myresults/$opts{node}_blocks.len$opts{pl}.ol$opts{ps}.synt -s $opts{tre} -n $opts{node} -m $opts{spenum2} -r blocks_list -t total > $opts{o2}/$opts{node}-myresults/$opts{node}_blocks.len$opts{pl}.ol$opts{ps}.taxonomy_filtered_total.synt";
