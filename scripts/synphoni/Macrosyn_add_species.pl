...
if (!(defined $opts{m} and defined $opts{n}  and defined $opts{a} and defined $opts{color} and defined $opts{ia} and defined $opts{g} and defined $opts{gff} and defined $opts{pep} and defined $opts{o1} and defined $opts{o2})) {
	...
	Optional:
	-evalue	Protein sequence alignment evalue (example:1e-5/0.001)
...
###########################################################33
if ($opts{a} eq "blast") {
	system "mkdir $opts{o2}/$opts{m}\_$opts{n}";
	print "The [$opts{o2}/$opts{m}\_$opts{n}] directory has been created!\n";
	...
	$fileExist = -e "$opts{o1}/add_species/$opts{m}/all.peps.psq";
	if ( $fileExist ) {
	}
	else {
	system "makeblastdb -in $opts{o1}/add_species/$opts{m}/all.peps -dbtype prot";
	}
	...
	my %h1=();my $score;
	open I,"< $opts{o2}/$opts{m}\_$opts{n}/$opts{n}_all.blast";
	while (my $a=<I>) {
		chomp $a;
		my @itms=split/\t/,$a;
		if (exists $h1{$itms[0]}) {
	...
	close I;
	my %h2=();my $score2;
	open I,"< $opts{o2}/$opts{m}\_$opts{n}/all_$opts{n}.blast";
	while (my $a=<I>) {
		chomp $a;
		my @itms=split/\t/,$a;
		if (exists $h2{$itms[0]}) {
	...
	my %h_gff=();
	open I,"<$opts{ia}/$opts{m}/$opts{m}\_simplified.gff" or die("Could not open $opts{ia}/$opts{m}/$opts{m}\_simplified.gff.\n"); 
	while (my $a=<I>) {
		chomp $a;
		my @itms=split/\t/,$a;
		$h_gff{$itms[1]}=0;
	...
	my %h_f2=();
	open I,"<$opts{o1}/add_species/$opts{m}/all.pairs2" or die("Could not open $opts{o1}/add_species/$opts{m}/all.pairs2.\n"); 
	while (my $a=<I>) {
		chomp $a;
		my @itms=split/\t/,$a;
		$h_f2{$itms[1]}=$itms[0];
	...
	open O,">$opts{o2}/$opts{m}\_$opts{n}/$opts{m}\_$opts{n}.pairs2";
	...
	my $spename1=$opts{m};
	my $spename2=$opts{n};
	my $pathway_gffm="$opts{ia}/$opts{m}/$opts{m}\_simplified.gff";
	my $pathway_gffn="$opts{gff}";
	my $pathway_b="$opts{ia}/$opts{m}/$opts{m}.block";
	get_matched_scaff1($spename1,$spename2,$pathway_gffm,$pathway_gffn);
	...
	sub get_matched_scaff1{
		...
		my %store=();
		my %nums=();
		open IN,"<$opts{o2}/$opts{m}\_$opts{n}/$opts{m}\_$opts{n}.pairs2" or die;
		while(<IN>){
			chomp;
			my @a=split /\t/;
		...
		close IN;
		open O,"> $opts{o2}/$opts{m}\_$opts{n}/$opts{m}\_$opts{n}.scaff.match";
	...
	sub sort_get_table1{
		...
		my %hash=();
		my %store2=();
		open IN,"<$opts{o2}/$opts{m}\_$opts{n}/$opts{m}\_$opts{n}.scaff.match" or die;
		while(my $a=<IN>){
			chomp $a;
			my @a=split/\t/,$a;
		...
		open O,"> $opts{o2}/$opts{m}\_$opts{n}/$opts{m}\_$opts{n}.scaff.match.table";
		my %final=();
	...
	my $pathway_pepm="$opts{ia}/$spename1/$spename1.pep";
	...
	if ($chromosome==1) {
		...
		while($num > $cutoff){
			...
			$n2=$n2+1;
			$c_value=$nv_cgn.$n2;
			testDB_scriptN_scaffold_clusteringb1($nv_cg,$i2,$cutoff,$n2,$c_value,$info);
		...
		scaff_c_formatted1($nv_cg,$final);
		...
		testDB_scriptN_scaffold_clusteringn1($pathway_genome,$spename2,$nv_cg,$info);
		...
		plot_dot_hd11($pathway_block,$spename1,$nv_cg,$spename2,$final);
		plot_dot_hd121($pathway_block,$spename1,$nv_cg,$spename2,$final);
	...
	if ($chromosome==2) {
		...
		plot_dot_hd21($pathway_block,$spename1,$nv_cg,$spename2,$pathway_genome);
		plot_dot_hd221($pathway_block,$spename1,$nv_cg,$spename2,$pathway_genome);
	...
	sub testDB_scriptN_scaffold_clustering1{
	...
	sub testDB_scriptN_scaffold_clusteringb1{
	...
	sub testDB_scriptN_scaffold_clusteringn1{
	...
	sub plot_dot_hd121{
		...
		system "Rscript Fish.R $pathway/$name $pathway/$nv_cg/sum-table";
		open I,"<$pathway/$nv_cg/$fi";
	...
	sub getseqpairv{
		system "mkdir $opts{o2}/$nv_cg/Macrosyn_genes";
		print "The [$opts{o2}/$nv_cg/Macrosyn_genes] directory has been created!\n";

		$c_chmun=$_[0];
		if ($c_chmun==1) {
			id_chr_match11($pathway_gffm,$pathway_block);
			...
			open G,"<$gff1";
			open O,">$opts{o2}/$nv_cg/Macrosyn_genes/$opts{m}.pair";
			while ($a=<G>){
				chomp $a;
				my @items=split/\t/,$a;
				open B,"<$bl";
			...
			sub c_scaffold21{
			open I1,"<$opts{o2}/$nv_cg/$spename2\_chr2scaf.info"; 
			$lv=1;
			while(<I1>){
				chomp;
				my @ar=split/\t/,$_;
			...
			close I1;
			open I2,"<$opts{o2}/$nv_cg/$spename2\_chr2scaf.info"; 
			open O,">$opts{o2}/$nv_cg/Macrosyn_genes/scaf_out"; 
			while(<I2>){
				chomp;
				my @ar2=split/\t/,$_;
			...
			open M,"<$opts{o2}/$nv_cg/Macrosyn_genes/$opts{m}.pair";
			while ($a=<M>){
				chomp $a;
				my @items=split/\t/,$a;
				$h1{$items[1]}=$items[0];
			}
			open I,"<$opts{o2}/$nv_cg/$spename1\_$spename2.pairs2";
			open O,">$opts{o2}/$nv_cg/Macrosyn_genes/pair3";
			while ($b=<I>){
				chomp $b;
				my @items2=split/\t/,$b;
				$b2=$items2[0];
			...
			sub id_chr_match211{
			open G,"<$pathway_gffn";
			open O,">$opts{o2}/$nv_cg/Macrosyn_genes/spe2.pair";
			while ($a=<G>){
				chomp $a;
				my @items=split/\t/,$a;
			...
			open I,"<$opts{o2}/$nv_cg/Macrosyn_genes/spe2.pair";
			open O,">$opts{o2}/$nv_cg/Macrosyn_genes/$opts{m}_$opts{n}_all_gene_pairs.dot";
			...
			open M,"<$opts{o2}/$nv_cg/Macrosyn_genes/scaf_out";
			while ($c=<M>){
				chomp $c;
				my @items2=split/\t/,$c;
			...
			open N,"<$opts{o2}/$nv_cg/Macrosyn_genes/pair3";
			while ($n=<N>){
				chomp $n;
				my @items4=split/\t/,$n;
				$i2=$items4[1];
			...
			system "rm $opts{o2}/$nv_cg/Macrosyn_genes/$opts{m}.pair";
			system "rm $opts{o2}/$nv_cg/Macrosyn_genes/scaf_out";
			system "rm $opts{o2}/$nv_cg/Macrosyn_genes/pair3";
			system "rm $opts{o2}/$nv_cg/Macrosyn_genes/spe2.pair";
			match_seqs11($opts{m},$opts{n});
			...
			open I,"<$opts{o2}/$nv_cg/Macrosyn_genes/$opts{m}_$opts{n}_all_gene_pairs.dot";
			open O1,">$opts{o2}/$nv_cg/Macrosyn_genes/$opts{m}_all_gene_pairs.pep";
			open O2,">$opts{o2}/$nv_cg/Macrosyn_genes/$opts{n}_all_gene_pairs.pep";
			while ($b=<I>){
				chomp $b;
				@items=split/\t/,$b;
			...
			sub get_dia_gene11{
				...
				close Y;
				system "awk -F '\t'  '{for(i=0;++i<=NF;)a[i]=a[i]?a[i] FS \$i:\$i}END{for(i=0;i++<NF;)print a[i]}' $opts{o2}/$nv_cg/$final > $opts{o2}/$nv_cg/Macrosyn_genes/try";
				my $max=0;my $num_max=0;my @items;my $num;my $n;my %h1;my %h2;my $fir;my @items2;my %h3;
				open IN,"<$opts{o2}/$nv_cg/Macrosyn_genes/try";
				open O,">$opts{o2}/$nv_cg/Macrosyn_genes/$opts{m}_$opts{n}_diagonal_gene_pairs.dot";
				open O1,">$opts{o2}/$nv_cg/Macrosyn_genes/$opts{m}_$opts{n}_significant_gene_pairs.dot";
				print O "Ancestor-geneID\tspecies-geneID\tAncestor-chr\tspecies-chr\n";
				...
				close IN;
				system "rm $opts{o2}/$nv_cg/Macrosyn_genes/try";
				open IN2,"<$opts{o2}/$nv_cg/Macrosyn_genes/$opts{m}_$opts{n}_all_gene_pairs.dot";
				while (my $a=<IN2>){
					chomp $a;
					@items2=split/\t/,$a;
			...
			open I,"<$opts{o2}/$nv_cg/Macrosyn_genes/$opts{m}_$opts{n}_diagonal_gene_pairs.dot";
			open O1,">$opts{o2}/$nv_cg/Macrosyn_genes/$opts{m}_diagonal_gene_pairs.pep";
			open O2,">$opts{o2}/$nv_cg/Macrosyn_genes/$opts{n}_diagonal_gene_pairs.pep";
			while ($b=<I>){
				chomp $b;
				@items=split/\t/,$b;
				if (exists $h1{$items[0]}) {
		...
		if ($c_chmun==2) {
			...
			close Y;
			id_chr_match12($pathway_gffm,$pathway_block);
			#system "perl  id_chr_match.pl -g /public/home/ws/ws_data/yuhw/synteny/connect86/gff/PY.chr.gff -b /public/home/ws/ws_data/yuhw/synteny/connect86/PY.block -o try";
			sub id_chr_match12{
			my $gff1=$_[0];my $bl=$_[1];my %h1;my %h2;
			open G,"<$gff1";
			...
			open I,"<$opts{o2}/$nv_cg/$nv_cg.pairs2";
			open O,">$opts{o2}/$nv_cg/Macrosyn_genes/pair3";
			while ($b=<I>){
				chomp $b;
				@items2=split/\t/,$b;
			...
			open M,"<$opts{o2}/$nv_cg/$spename2\_chr2scaf.info";
			while ($c=<M>){
				chomp $c;
				@items2=split/\t/,$c;
				foreach my $ii(@items2) {
			...
			#system "perl get_dia_gene.pl -matrix /public/home/ws/ws_data/yuhw/synteny/connect86/result2/PY_CG/PY_CG_scaff_c3 -o /public/home/ws/ws_data/yuhw/synteny/connect86/result/tiqv";
			sub get_dia_gene12{
			system "awk -F '\t'  '{for(i=0;++i<=NF;)a[i]=a[i]?a[i] FS \$i:\$i}END{for(i=0;i++<NF;)print a[i]}' $opts{o2}/$nv_cg/$nv_cg.scaff.match.table > $opts{o2}/$nv_cg/Macrosyn_genes/try";
			my $max=0;my $num_max=0;my @items;my $num;my $n;my %h1;my %h2;my $fir;my @items2;my %h3;
			open IN,"<$opts{o2}/$nv_cg/Macrosyn_genes/try";
			open O,">$opts{o2}/$nv_cg/Macrosyn_genes/$opts{m}_$opts{n}_diagonal_gene_pairs.dot";
			print O "Ancestor-geneID\tspecies-geneID\tAncestor-chr\tspecies-chr\n";
	...
	print "The [$pathway/$nv_cg/chr_breakage_fusion_result] directory has been created!\n";
	...
	open O,">$opts{o2}/formatted_ancestor_chr_color.txt";
	my %h_un=();my %h_numpai=();
...
if ($opts{a} eq "diamond") {
	...
	$fileExist = -e "$opts{pep}.database.dmnd";
	if ( $fileExist ) {
	}
	else {
	system "diamond makedb --in $opts{pep} -d $opts{pep}.database";
	}

	$fileExist = -e "$opts{o1}/add_species/$opts{m}/all.peps.database.dmnd";
	if ( $fileExist ) {
	}
	else {
	system "diamond makedb --in $opts{o1}/add_species/$opts{m}/all.peps -d $opts{o1}/add_species/$opts{m}/all.peps.database";
	}
	...
	if ($chromosome==2) {
		...
		plot_dot_hd22($pathway_block,$spename1,$nv_cg,$spename2,$pathway_genome);
		#system"perl scripts/plot_dot_hd.pl $pathway_block/$spename1.block $pathway_genome/$spename2.fa.len $pathway/$nv_cg/$nv_cg.scaff.match.table $pathway/$nv_cg/$nv_cg";
	}
	...
	sub getseqpair{
		...
		if ($c_chmun==2) {
			...
			close Y;
			id_chr_match13($pathway_gffm,$pathway_block);
			#system "perl  id_chr_match.pl -g /public/home/ws/ws_data/yuhw/synteny/connect86/gff/PY.chr.gff -b /public/home/ws/ws_data/yuhw/synteny/connect86/PY.block -o try";
			sub id_chr_match13{
			my $gff1=$_[0];my $bl=$_[1];my %h1;my %h2;
			open G,"<$gff1";
