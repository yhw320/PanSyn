...
while ($aa=readdir I) {
	if ( $aa=~/^(\S+).pep$/ ) {
		...
		if ($spe_name ne $opts{a}) {
	        ...
	        system "blastp -query $opts{i}/$opts{a}.pep -db $file -outfmt 6 -out $opts{o}/$opts{a}_$spe_name/$opts{a}_$spe_name.blast  -evalue $opts{e} -num_threads $opts{t} -max_target_seqs $opts{max}";
