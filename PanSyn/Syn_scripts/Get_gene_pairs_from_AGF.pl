...
use Getopt::Long;
my %opts;
GetOptions(\%opts,"i=s","m=s","n=s","o=s","o2=s","e=s","h|help");
if (!(defined $opts{i} and defined $opts{m} and defined $opts{n} and defined $opts{o} and defined $opts{o2})) {
		die "************************************************\n
	-i	Full path to the folder containing required data files (same as [-i] in the previous script [Macrosyn1_1_blast.pl])
	-m	Selected Ancestor' species by name abbreviations (example: NVec)
	-n	Interested species by name abbreviations (example: HSap)
	-o	Full path to the folder storing the output results (same as [-o] in the previous script [Macrosyn1_1_blast.pl])
	-o2	Full path to the new folder storing the output results
	Optional:
	-e	Alignment evalue (default:1e-5)
...
system"cat $pathway/blastp/$spename1.blastp $pathway/blastp/$spename2.blastp > $opts{o2}/$nv_cg.blastp";
...
sub get_pairs_from_cluster {
    ...
    my $p2=$_[2];
    my %matchs=();
    open IN,"<","$opts{o2}/$one.blastp" or die;
    while(<IN>){
	    chomp;
	    my @a=split /\t/;
    ...
    open O,"> $opts{o2}/$one.pairs.score";#NV_GG.pairs
    while(<IN>){
	    chomp;
	    s/,$//;
	    my @a=split /\t/;
...
sub synteny_match_id{
    ...
    open Q,"< $opts{o2}/$name.pairs.score";#NV_GG.pairs
    open O,"> $opts{o2}/$name.pairs2.score";#NV_GG.pairs2
    while (my $a=<Q>) {
	    chomp $a;
	    my @itms=split/\t/,$a;
