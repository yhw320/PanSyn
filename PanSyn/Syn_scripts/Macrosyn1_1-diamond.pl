...
if (!( defined $opts{i} and defined $opts{o})) {
		die "************************************************\n
	-i	Full path to the folder containing required data files
...
system "mkdir $opts{o}/ids_match";
system "mkdir $opts{o}/score";
system "mkdir $opts{o}/single_cluster";
system "mkdir $opts{o}/blastp";
system "mkdir $opts{o}/formatted_peps";
...
sub testDB_script1_get_formatted_input_pep_file {
    ...
    open OUT1,"> $opts{o}/formatted_peps/$spe.pep.fa";
    open OUT2,"> $opts{o}/ids_match/$spe.ids.match";
    while (my $a=<IN>) {
	    chomp $a;
	    if ($a=~/>(\S+)/) {
		    $n++;
...
###################################################
system "cat $opts{o}/formatted_peps/*fa > $opts{o}/formatted_peps/all.fa";
$fileExist = -e "$opts{o}/formatted_peps/all.fa.database.dmnd";
if ( $fileExist ) {
}
else {
system "diamond makedb --in $opts{o}/formatted_peps/all.fa -d $opts{o}/formatted_peps/all.fa.database";
}


my $dir2=$opts{o}."/formatted_peps";
opendir F,$dir2;
while (my $d=readdir F) {
	if ( $d=~/^(\S+).pep.fa$/ ) {
		my $file2=$dir2."/$d";
        system "diamond blastp --query $opts{o}/formatted_peps/$1.pep.fa --db $opts{o}/formatted_peps/all.fa.database --outfmt 6 --out $opts{o}/blastp/$1.blastp  --evalue $opts{e} --threads $opts{t}";
		system"less $opts{o}/blastp/$1.blastp |awk '{print \$1\"\t\"\$2\"\t\"\$12}'|sort -k 1 -nk 3 > $opts{o}/score/$1.score";
    	cluster_out($1); 
	}
}
close F;
...
sub cluster_out{
	my $n=0;my %h_sub;
    my $spename=$_[0];
    open IN,"<$opts{o}/score/$spename.score";
    while (my $a=<IN>){
	    chomp $a;
	    my @items=split/\t/,$a;
   	    $h_sub{$items[0]}=1;
    ...
    open OUT,">$opts{o}/single_cluster/$spename.cluster2";
...
system"mkdir $opts{o}/cluster1";
system"mkdir $opts{o}/table";

open T5,"<$opts{i}/tree.nwk" or die("Could not open $opts{i}/tree.nwk\n"); 
while (my $a=<T5>){
    chomp $a;
	$a=~s/;//g;
	while ($a=~/,/) {
		...
		foreach $pair (@b) {
			...
			$spe2=$single_spe[1];
			$addname=$spe1."_".$spe2;
            system"cat $opts{o}/score/$spe1.score $opts{o}/score/$spe2.score > $opts{o}/score/$addname.score";
	...
	my $path_cate=$opts{i}."/species_classification.table";
...
sub get_ancietS{
    ...
    open I,"<$opts{o}/single_cluster/$cl.cluster2" or die;
    open OUT,">$opts{o}/ancient_gene.families";
    while(<I>){
	    chomp;
	    my %count=();
	    s/,$//;
...
sub count_inter_outer_scores{
    ...
    my %outMaxScore=();
    my %interMaxScore=();
    open IN,"<","$opts{o}/cluster1/$aname.cluster1" or die;
    while(<IN>){
	    chomp;
	    s/,$//;
	    my @a=split /\t/;
    ...
    close IN;
    open IN,"<","$opts{o}/score/$aname.score" or die;
    while(<IN>){
	    chomp;
	    my @a=split /\t/;
	    next if ($a[1] eq $a[0]);
    ...
    my %pres=();
    open OUT,">$opts{o}/table/$aname.cluster1.table";
...
sub MacroSystenic_firstCluster{
    ...
    my %hCluster2=();
#====================================
    open IN,"<","$opts{o}/single_cluster/$spea.cluster2" or die;
    while(<IN>){
	    chomp;
	    s/,$//;
	    my @a=split /\t/;
    ...
    close IN;
#====================================
    open IN,"<","$opts{o}/single_cluster/$speb.cluster2" or die;
    while(<IN>){
	    chomp;
	    s/,$//;
	    my @a=split /\t/;
...
#=======================================
    my %ScoreStore1=();
    my %ScoreStore2=();
    open IN,"<","$opts{o}/score/$adname.score" or die;
    while(<IN>){
	    chomp;
	    my @a=split /\t/;
...
#========================================================
    open OUT,">$opts{o}/cluster1/$adname.cluster1";
...
sub MacroSystenic_step2{
	my $aname2=$_[0];
    my %genes=();
    open IN,"<","$opts{o}/cluster1/$aname2.cluster1" or die;
    while(<IN>){
	    chomp;
	    s/,$//;
	    my @a=split /\t/;
    ...
    my %matchs=();
    open IN,"<","$opts{o}/table/$aname2.cluster1.table" or die;
    while(<IN>){
	    chomp;
	    my @a=split /\t/;
	    $matchs{$a[0]}{$a[1]}=1;
    ...
    close IN;
    open OUT,">$opts{o}/single_cluster/$aname2.cluster2";
    my %exi=();
...
sub caculate_family{
	my $cll=$_[0];
	open I,"<$opts{o}/ancient_gene.families" or die("Could not open $opts{o}/ancient_gene.families\n"); 
    my $line = "";
    my %ancient_spes_num = ();
    my $total_ancient = 0;
    while($line = <I>){
    ...
    open D,"<$opts{o}/single_cluster/$cll.cluster2" or die;
    my $line1 = "";
    ...
    close D;
    open O,">$opts{o}/gene_families.situation";
    ...
    close O;
    open C,">$opts{o}/family_situation.table";
...
system("mkdir $opts{o}/genes-in-ancient_gene_families");
...
foreach my $i (sort keys %hg) {
	...
	close I2;
	open I,"<$opts{o}/ids_match/$i.ids.match";
	my %h_xp=();
	while(my $a = <I>){
		chomp $a;my @xp=();
		@xp=split/\t/,$a;
	...
	close I;
	open O,">$opts{o}/genes-in-ancient_gene_families/$i.ancient_gene_sequences";
	foreach my $m (@gr3) {
		if (exists $h_xp{$m}) {
			my $mmm=$h_xp{$m};
			if (exists $h_seq{$mmm}) {
...
system("mkdir $opts{o}/ancient_gene_families.analysis");
system("mv $opts{o}/single_cluster/$addname.cluster2 $opts{o}/ancient_gene_families.analysis/all_gene.families");
system("mv $opts{o}/ancient_gene.families $opts{o}/ancient_gene_families.analysis/ancient_gene.families");
system("mv $opts{o}/gene_families.situation $opts{o}/ancient_gene_families.analysis/gene_families.situation");
system("mv $opts{o}/ancient_gene_families.analysis/gene_families.situation $opts{o}/ancient_gene_families.analysis/gene_families_summary.table");
system "mv $opts{o}/genes-in-ancient_gene_families $opts{o}/ancient_gene_families.analysis/ancient_gene_families.genes";
system "mv $opts{o}/family_situation.table $opts{o}/ancient_gene_families.analysis/family_situation.table";
