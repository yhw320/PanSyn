...
$fileExist = -e "$opts{o}/formatted_peps/all.fa.psq";
if ( $fileExist ) {
}
else {
	system "makeblastdb -in $opts{o}/formatted_peps/all.fa -dbtype prot";
}
...
while (my $d=readdir F) {
	if ( $d=~/^(\S+).pep.fa$/ ) {
		my $file2=$dir2."/$d";
		system "blastp -query $opts{o}/formatted_peps/$1.pep.fa -db $opts{o}/formatted_peps/all.fa -outfmt 6 -out $opts{o}/blastp/$1.blastp -evalue $opts{e} -num_threads $opts{t}";
