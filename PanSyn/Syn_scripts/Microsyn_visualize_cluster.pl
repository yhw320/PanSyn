...
use Getopt::Long;
my %opts;
GetOptions(\%opts,"i=s","o=s","b=s","c=s","color=s","s=s","width=s","height=s","h|help");
if(!(defined $opts{i} and defined $opts{o} and defined $opts{b} and defined $opts{c} and defined $opts{color} and defined $opts{s})){
	die"**********************************************\n
options:
	-i	Full path to the [inputDir] folder containing input files (same as the input file path [-i] in the previous script Microsyn_cluster-blast.pl)
	-o	Full path to the new [outputDir] folder containing output files	
	-b	Full path to the selected micro-synteny gene block file (\*_micorsyn_genes.blocks generated by the previous script Microsyn_cluster-blast.pl)
...
close I;
system "mkdir $opts{o}/visualize_cluster";
...
close I;
open O,">$opts{o}/visualize_cluster/allspe_conserved.clusters";
...
###hox2.pl
open IN,"<$opts{o}/visualize_cluster/allspe_conserved.clusters";
open O,">$opts{o}/visualize_cluster/forR_microsyn_conserved.cluster";
print O "name\tstart\tend\tstrand\tcol\tfill\tGenome\tgene_type\n";

while (my $a=<IN>){
	chomp $a;
	my @it1=split/\t/,$a;
	open G,"<$opts{i}/$it1[0].gff";
	%h_gene=();
	while (my $b=<G>){
		chomp $b;
		my @it2=split/\t/,$b;
	...
	close G;
	system "sort -t \$'\t' -k 1,1 -k 3n,3  $opts{i}/$it1[0].gff >$opts{i}/$it1[0].gff_sort";
...
open I1,"<$opts{o}/visualize_cluster/forR_microsyn_conserved.cluster";
open O,">$opts{o}/visualize_cluster/inter_forR_microsyn_conserved.cluster";
open I2,"<$opts{color}";
while ($a=<I2>){
	chomp $a;
	my @itt=split/\t/,$a;
...
open INN,"<$opts{o}/visualize_cluster/inter_forR_microsyn_conserved.cluster";
open O,">$opts{o}/visualize_cluster/final_forR_microsyn_conserved.cluster";
...
system "Rscript $opts{s}/Microsyn_visualize_cluster.R $opts{o}/visualize_cluster $opts{o}/visualize_cluster/final_forR_microsyn_conserved.cluster $opts{width} $opts{height}";