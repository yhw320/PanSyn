...
foreach my $i (sort {$a<=>$b} keys %h) {
	for ($n1=$i+1;$n1<=$num;$n1=$n1+1) {
		if ($i!=$num) {
			...
			system "blastp -query $opts{i}/$h{$i}.pep -db $opts{i}/$h{$n1}.pep -outfmt 6 -out $opts{o}/$h{$i}_$h{$n1}/$h{$i}_$h{$n1}.blast -evalue $opts{e} -num_threads $opts{t} -max_target_seqs 5";
